...
while((m=ir.exec(clause.body))!==null){let t=normalizeTerm(m[2]);if(/^the\s+/i.test(t))t=t.replace(/^the\s+/i,'');if(!t||isBadDefinitionTerm(t)||/\b(?:console|function|javascript|script|alert|onerror)\b/i.test(m[1]))continue;const cs=sentences.find(s=>s.includes(m[0]))||m[0];const parenInSentence=cs.match(new RegExp(`\\(\\s*(?:the\\s+)?["“”'‘’]${escapeRegExp(m[2])}["“”'‘’]\\s*\\)`,'i'));const precedingInSentence=parenInSentence?cs.slice(0,parenInSentence.index):m[1];const c=cleanInlineCandidate((precedingInSentence||m[1]).trim())||cleanInlineCandidate(m[1].trim());addTerm({term:t,normalized:t,definition:c,candidateDefinition:c,contextSentence:cs,definitionType:'inline',confidence:'medium',confidenceRank:2+hb,detectionRule:'inline_parenthetical_term',definedInClauseId:clause.id,usedInClauseIds:[],scope});}
