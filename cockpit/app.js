...
async function analyzeText(text,fileName,sourceType,extras={}){
  ...
  try {
    ...
    state.issues.crossReferenceBreaks=state.referenceLedger.filter(item=>item.status==='missing'||item.status==='ambiguous');
...
function splitNumberedClauseContent(parsed,line){
  ...
  return{heading:conciseClauseHeading(content,parsed?.number||''),body:normalizeDocxLine(line).replace(/^\s*(?:Schedule\s+[A-Z0-9]+|Annex(?:ure)?\s+[A-Z0-9]+|Exhibit\s+[A-Z0-9]+|Appendix\s+[A-Z0-9]+|(?:Article|Section|Clause)\s+[A-Z0-9.-]+|\d+(?:\.\d+)*(?:[a-z])?(?:\([a-z0-9ivx]+\))*|[A-Z]|[ivxlcdm]{1,8})[.)]?\s*/i,'')};
}
...
function isBadDefinitionTerm(t,context=''){if(typeof ANALYSIS_CORE.assessDefinedTermCandidate==='function')return !ANALYSIS_CORE.assessDefinedTermCandidate(t,context).include;return /^(The|This|That|Any|Each|Either|For|If|In|On|At|From|To|And|Or)$/i.test(t);}
...
formalPatterns.forEach(({regex,type,confidence,rank,rule})=>{let m;while((m=regex.exec(clause.body))!==null){let t=normalizeTerm(m[1]);if(!t||isBadDefinitionTerm(t))continue;const cs=sentences.find(s=>s.includes(m[0]))||m[0];const def=m[2].trim();addTerm({term:t,normalized:t,definition:type==='negative-definition'?'':(type==='cross_reference'?`See ${def}`:def),carveOuts:type==='negative-definition'?[def]:[],reference:type==='cross_reference'?def:'',contextSentence:cs,definitionType:type,confidence,confidenceRank:rank+hb,detectionRule:rule,definedInClauseId:clause.id,usedInClauseIds:[],scope});}});
let m;const ir=/([^\n\r()]{2,180}?)\s*\(\s*(?:the\s+)?["“”'‘’]([A-Z][A-Za-z0-9&/ -]{1,80})["“”'‘’]\s*\)/gi;
while((m=ir.exec(clause.body))!==null){let t=normalizeTerm(m[2]);if(/^the\s+/i.test(t))t=t.replace(/^the\s+/i,'');if(!t||isBadDefinitionTerm(t)||/\b(?:console|function|javascript|script|alert|onerror)\b/i.test(m[1]))continue;const c=cleanInlineCandidate(m[1].trim());const cs=sentences.find(s=>s.includes(m[0]))||m[0];addTerm({term:t,normalized:t,definition:c,candidateDefinition:c,contextSentence:cs,definitionType:'inline',confidence:'medium',confidenceRank:2+hb,detectionRule:'inline_parenthetical_term',definedInClauseId:clause.id,usedInClauseIds:[],scope});}
...
while((m=rr.exec(clause.body))!==null){const t=normalizeTerm(m[2]);if(!t||isBadDefinitionTerm(t))continue;const cs=sentences.find(s=>s.includes(m[0]))||m[0];const before=clause.body.slice(0,m.index);const phrase=cleanInlineCandidate(before.slice(Math.max(0,before.length-220)));addTerm({term:t,normalized:t,definition:phrase,candidateDefinition:phrase,contextSentence:cs,definitionType:'inline',confidence:'medium',confidenceRank:2+hb,detectionRule:'referred_to_as_term',definedInClauseId:clause.id,usedInClauseIds:[],scope});}
const hr=/\bhereinafter\s*(?:(?:collectively\s+|jointly\s+)?referred\s+to\s+as)?\s*:?\s*(?:the\s+)?["“”'‘’]([A-Z][A-Za-z0-9&/ -]{1,80})["“”'‘’]/gi;
while((m=hr.exec(clause.body))!==null){const t=normalizeTerm(m[1]).replace(/^the\s+/i,'');if(!t||isBadDefinitionTerm(t)||/^hereinafter/i.test(t))continue;const cs=sentences.find(s=>s.includes(m[0]))||m[0];const before=clause.body.slice(0,m.index);const phrase=cleanInlineCandidate(before.slice(Math.max(0,before.length-260)));addTerm({term:t,normalized:t,definition:phrase||'Defined label in the preamble.',candidateDefinition:phrase,contextSentence:cs,definitionType:'inline',confidence:'high',confidenceRank:4,detectionRule:'hereinafter_parenthetical_or_colon',definedInClauseId:clause.id,usedInClauseIds:[],scope});}
});
